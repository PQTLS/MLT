...
# endif                         /* OPENSSL_NO_DES */
    ...
    for (j = 0; j < OQSKEM_NUM; j++) {
        ...
        */
            oqskem_public_key = (uint8_t *)aligned_alloc(32, oqskem_kem[j]->length_public_key);
		    oqskem_secret_key = (uint8_t *)aligned_alloc(32, oqskem_kem[j]->length_secret_key);
		    oqskem_ciphertext = (uint8_t *)aligned_alloc(32, oqskem_kem[j]->length_ciphertext);
		    oqskem_shared_secret_e = (uint8_t *)aligned_alloc(32, oqskem_kem[j]->length_shared_secret);
		    oqskem_shared_secret_d = (uint8_t *)aligned_alloc(32, oqskem_kem[j]->length_shared_secret);
