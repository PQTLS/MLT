...
static int add_key_share(SSL *s, WPACKET *pkt, unsigned int curve_id)
{
    ...
    if (s->s3->tmp.pkey != NULL) {
      ...
      if (do_pqc || do_hybrid) {
        ...
        if ((oqs_encoded_point = (uint8_t *)aligned_alloc(32,s->s3->tmp.oqs_kem->length_public_key)) == NULL ||
            (s->s3->tmp.oqs_kem_client = (uint8_t *)aligned_alloc(32,s->s3->tmp.oqs_kem->length_secret_key)) == NULL ||
                             ...
                             size_t chainidx)
{
    ...
    if (do_pqc || do_hybrid) {
        ...
        if ((oqs_shared_secret = (uint8_t *)aligned_alloc(32,s->s3->tmp.oqs_kem->length_shared_secret)) == NULL ||
