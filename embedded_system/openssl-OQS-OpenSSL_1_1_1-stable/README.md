...
The following instructions will download and build liboqs, then install it into a subdirectory inside the OpenSSL folder.
	...
	cmake -GNinja -DCMAKE_INSTALL_PREFIX=/home/pqc/OQS/openssl-OQS-OpenSSL_1_1_1_modify/oqs -DOQS_USE_OPENSSL=OFF ..
	ninja
	ninja install
...
and specify the key-exchange algorithm through `s_time` using `apps/openssl s_time -curves <KEX>`.
	...
	/**************************************例子
	apps/openssl req -x509 -new -newkey rsa:2048 -keyout rootCA.key -out rootCA.crt -nodes -subj "/CN=oqstest CA" -days 365 -config apps/openssl.cnf

	apps/openssl req -new -newkey rsa:2048 -keyout server.key -out server.csr -nodes -subj "/CN=oqstest server" -config apps/openssl.cnf

    apps/openssl x509 -req -in server.csr -out server.crt -CA rootCA.crt -CAkey rootCA.key -CAcreateserial -days 365

    apps/openssl s_server -cert server.crt -key server.key -www -tls1_3

    //apps/openssl s_client -groups kyber768 -CAfile rootCA.crt

	apps/openssl s_time -curves kyber768
