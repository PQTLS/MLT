...
ip link add veth3 type veth peer name veth9
ip link add veth4 type veth peer name veth10
ip link add veth5 type veth peer name veth11
ip link add veth6 type veth peer name veth12
ip link add veth7 type veth peer name veth13
ip link add veth8 type veth peer name veth14
...
ip link set veth5 netns node2
ip link set veth6 netns node2
ip link set veth7 netns node2
ip link set veth8 netns node2

ip link set veth9 netns node3
...
ip netns exec node2 ip link set dev veth5 up
ip netns exec node2 ip link set dev veth6 up
ip netns exec node2 ip link set dev veth7 up
ip netns exec node2 ip link set dev veth8 up


ip netns exec node3 ip link set dev lo up
ip netns exec node3 ip link set dev veth9 up
...
ip netns exec node2 ip addr add 192.168.4.1/24 dev veth5
ip netns exec node2 ip addr add 192.168.5.1/24 dev veth6
ip netns exec node2 ip addr add 192.168.6.1/24 dev veth7
ip netns exec node2 ip addr add 192.168.7.1/24 dev veth8

ip netns exec node3 ip addr add 192.168.2.2/24 dev veth9

ip netns exec node4 ip addr add 192.168.3.2/24 dev veth10
...
ip netns exec node3 ip route add 192.168.1.0/24 via 192.168.2.1 dev veth9
ip netns exec node1 ip route add 192.168.2.0/24 via 192.168.1.2 dev veth1
ip netns exec node4 ip route add 192.168.1.0/24 via 192.168.3.1 dev veth10
...
sudo ip netns exec node2 tc qdisc add dev veth5 root netem
sudo ip netns exec node2 tc qdisc add dev veth6 root netem
sudo ip netns exec node2 tc qdisc add dev veth7 root netem
sudo ip netns exec node2 tc qdisc add dev veth8 root netem

sudo ip netns exec node3 tc qdisc add dev veth9 root netem
