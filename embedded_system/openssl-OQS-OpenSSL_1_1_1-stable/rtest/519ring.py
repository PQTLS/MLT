...
def test_node(node_id,latency_time,ke_alg,lossrate,current_time):
    count = 500
    folder_path = os.path.join("524ring", 'node'+str(node_id)+'_'+current_time)
...
def run_timers(kex_alg,latency_time,lossrate,current_time, timer_pool):
    timer_pool.starmap(test_node, [(2,latency_time,kex_alg,lossrate,current_time),(3,latency_time,kex_alg,lossrate,current_time),(4,latency_time,kex_alg,lossrate,current_time),
                                   (5,latency_time,kex_alg,lossrate,current_time),(6,latency_time,kex_alg,lossrate,current_time),(7,latency_time,kex_alg,lossrate,current_time)])
...
if __name__ == "__main__":
    ...
    for i in range(0,3):
        ...
        lossrate = Lossrate[i]
        netem_set('node1', 'veth15', lossrate, latency=latency_time)
        ...
        netem_set('node3', 'veth6', lossrate, latency=latency_time)
        netem_set('node4', 'veth7', lossrate, latency=latency_time)
        netem_set('node4', 'veth8', lossrate, latency=latency_time)
        netem_set('node5', 'veth9', lossrate, latency=latency_time)
        netem_set('node5', 'veth10', lossrate, latency=latency_time)
        netem_set('node6', 'veth11', lossrate, latency=latency_time)
        netem_set('node6', 'veth12', lossrate, latency=latency_time)
        netem_set('node7', 'veth13', lossrate, latency=latency_time)
        netem_set('node7', 'veth14', lossrate, latency=latency_time)
